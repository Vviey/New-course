...
export default function StoryIntro() {
  ...
  const storyPages = [
    {
      ...
      pattern: 'adinkra',
      symbol: '🌍' // Earth symbol representing beginnings
    },
    {
    ...
    },
    {
      ...
      pattern: 'mudcloth',
      symbol: '✨' // Sparkles representing magic/beginning of journey
    }
  ];
  ...
  const handleNextPage = () => {
    if (currentPage < storyPages.length - 1) {
      ...
      // Navigate to realm home page when story is complete
  ...
  // Design elements inspired by African textiles
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={pagePatternStyle}
      />
      
      {/* Content container with adinkra-inspired border */}
      <div className="max-w-3xl w-full bg-amber-900/80 backdrop-blur-sm p-8 rounded-xl shadow-xl z-10 relative">
        {/* Decorative border with adinkra-inspired patterns */}
        <div className="absolute inset-0 rounded-xl border-4 border-amber-500/60" style={{
          backgroundImage: 'linear-gradient(45deg, transparent 30%, #FFC567 30%, #FFC567 70%, transparent 70%), linear-gradient(-45deg, transparent 30%, #FFC567 30%, #FFC567 70%, transparent 70%)',
          backgroundSize: '20px 20px',
          opacity: 0.2,
          zIndex: -1
        }}></div>
          ...
          <div className="w-full md:w-2/5 flex justify-center">
            ...
            >
              ...
              <div className="absolute inset-0 rounded-full" style={{
                backgroundImage: 'radial-gradient(circle, transparent 30%, rgba(0,0,0,0.2) 30%, rgba(0,0,0,0.2) 40%, transparent 40%), radial-gradient(circle, rgba(0,0,0,0.2) 20%, transparent 20%)',
        ...
        <div className="flex justify-between items-center">
          <div className="flex space-x-3">
            {storyPages.map((_, idx) => (
              ...
              >
                <div 
                  className={`w-4 h-4 transform rotate-45 ${idx === currentPage ? 'bg-amber-300' : 'bg-amber-700'}`}
                />
                {idx === currentPage && (
                  <div className="absolute inset-0 w-4 h-4 transform rotate-45 bg-amber-300 animate-pulse" 
                    style={{opacity: 0.5}}
                  />
                )}
              </div>
