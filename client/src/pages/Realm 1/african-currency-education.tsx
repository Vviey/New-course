...
export default function AfricanCurrencyEducation({ onComplete }: MissionProps) {
  ...
  // African pattern styles based on the image
  const patternStyles = {
    spirals: {
    ...
    waves: {
      backgroundImage: `linear-gradient(135deg, #EE720B 25%, transparent 25%) , 
                        linear-gradient(225deg, #EE720B 25%, transparent 25%) , 
                        linear-gradient(45deg, #EE720B 25%, transparent 25%) , 
                        linear-gradient(315deg, #EE720B 25%, transparent 25%)`,
    ...
    chevron: {
      backgroundImage: `linear-gradient(135deg, #FFC567 25%, transparent 25%) , 
                        linear-gradient(225deg, #FFC567 25%, transparent 25%)`,
      backgroundSize: '30px 30px',
      backgroundColor: '#EE720B',
      borderColor: '#FBF4D2'
    },
    floral: {
      backgroundImage: `radial-gradient(circle at 50% 50%, #FFC567 10%, transparent 30%),
                        radial-gradient(circle at 100% 50%, #EE720B 20%, transparent 30%),
                        radial-gradient(circle at 0% 50%, #EE720B 20%, transparent 30%),
                        radial-gradient(circle at 50% 100%, #EE720B 20%, transparent 30%),
                        radial-gradient(circle at 50% 0%, #EE720B 20%, transparent 30%)`,
  ...
  const nextMission = () => {
    if (currentMission < missions.length - 1) {
      ...
      // Mark previous mission as completed
      if (!completedMissions.includes(missions[currentMission].id)) {
        setCompletedMissions([...completedMissions, missions[currentMission].id]);
      }
    }
    
    // If we've reached the end, call onComplete
    if (currentMission === missions.length - 2 && onComplete) {
      // Add last mission to completed list
      if (!completedMissions.includes(missions[currentMission + 1].id)) {
        setCompletedMissions(prev => [...prev, missions[currentMission + 1].id]);
      }
      
      // Allow time to read the final mission content before triggering completion
      setTimeout(() => {
        onComplete();
      }, 3000);
    }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-amber-50">
      ...
      <div className="mb-6">
        ...
        <div className="bg-white p-6 rounded-b-lg shadow-md border-l-4 border-r-4 border-b-4" style={{ borderColor: patternStyles[mission.pattern].borderColor }}>
